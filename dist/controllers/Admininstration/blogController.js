...
async function createBlog(request, response) {
    ...
    try {
        ...
        const validationRules = [
            (0, express_validator_1.body)('title').notEmpty().withMessage('Title is required').isLength({ min: 3 }).withMessage('Title must be at least 3 characters'),
            (0, express_validator_1.body)('content').notEmpty().withMessage('Content is required'),
            (0, express_validator_1.body)('category_id').notEmpty().withMessage('Category ID is required').isInt().withMessage('Category ID must be an integer'),
            (0, express_validator_1.body)('tag_id').notEmpty().withMessage('Tag ID is required').isInt().withMessage('Tag ID must be an integer'),
            (0, express_validator_1.body)('published').optional().isBoolean().withMessage('Published must be a boolean')
        ];
        await Promise.all(validationRules.map((rule) => rule.run(request)));
        ...
        if (request.file) {
            ...
            try {
                // Upload image to Cloudinary
                uploadedImageUrl = await (0, cloudinary_1.uploadImage)(image_path, 'legasi/images/blog_images/');
                ...
                if (fs_1.default.existsSync(image_path)) {
                    fs_1.default.unlinkSync(image_path);
                }
...
async function updateBlog(request, response) {
    try {
        const admin_id = request.user.adminId;
        const id = parseInt(request.query.blog_id, 10);
        if (!admin_id) {
            return response.status(403).json({ message: 'Unauthorized User' });
        }
        // Check admin role
        ...
        // Parse form data
        const { title, content, category_id, tag_id, published } = request.body;
        const cover_image_file = request.file; // Use type assertion for multer file
        const validationRules = [
            (0, express_validator_1.body)('title').optional().isLength({ min: 3 }).withMessage('Title must be at least 3 characters'),
            (0, express_validator_1.body)('category_id').optional().isInt().withMessage('Category ID must be an integer'),
            (0, express_validator_1.body)('tag_id').optional().isInt().withMessage('Tag ID must be an integer'),
            (0, express_validator_1.body)('published').optional().isBoolean().withMessage('Published must be a boolean')
        ];
        await Promise.all(validationRules.map((rule) => rule.run(request)));
        const errors = (0, express_validator_1.validationResult)(request);
        ...
        if (cover_image_file) {
            ...
            const uploadResult = await new Promise((resolve, reject) => {
                const uploadStream = cloudinary_2.v2.uploader.upload_stream({
                    folder: 'blog-covers',
                    transformation: [
        ...
        if (coverImageUrl) {
            ...
            if (existingBlog?.cover_image) {
                try {
                    ...
                    if (publicId) {
                        await cloudinary_2.v2.uploader.destroy(`blog-covers/${publicId}`);
                    }
                }
                catch (deleteError) {
...
async function allBlogs(request, response) {
    const admin_id = request.user.adminId;
    if (!admin_id) {
        return response.status(403).json({ message: 'Unauthorized User' });
    }
...
async function singleBlog(request, response) {
    const id = parseInt(request.query.blog_id, 10);
    const admin_id = request.user.adminId;
    if (!admin_id) {
        return response.status(403).json({ message: 'Unauthorized User' });
